...
import numpy as np
from src.tracker.imu_buffer import ImuBuffer
from src.tracker.imu_calib import ImuCalib
from src.tracker.meas_source_torchscript import MeasSourceTorchScript
from src.tracker.scekf import ImuMSCKF
from src.utils.dotdict import dotdict
from src.utils.from_scipy import compute_euler_from_matrix
from src.utils.logging import logging
from src.utils.math_utils import mat_exp
...
class ImuTracker:
    ...
    def _get_imu_samples_for_network(self, t_begin_us, t_oldest_state_us, t_end_us):
        ...
        # net_acc, net_gyr, net_tus = self.imu_buffer.get_last_k_data(200)
        assert net_gyr.shape[0] == self.net_input_size
        assert net_acc.shape[0] == self.net_input_size
        # get data from filter
        state_idx = self.filter.state.si_timestamps_us.index(t_oldest_state_us)
        R_oldest_state_wfb = self.filter.state.si_Rs[state_idx]
    ...
    def _on_imu_measurement_after_init(self, t_us, gyr_raw, acc_raw):
        ...
        # x = self.imu_buffer.net_acc.shape
        # y = do_augmentation_and_update
        # filter update
        ...
        if do_augmentation_and_update:
            ...
            # print("do_augmentation_and_update time: ", time.time() - ts)
    ...
    def _process_update(self, t_us):
        ...
        if self.debug_callback_get_meas:
            ...
            # print("data time: ", ts2 - ts1, "net time: ", time.time() - ts2)
        # filter update
