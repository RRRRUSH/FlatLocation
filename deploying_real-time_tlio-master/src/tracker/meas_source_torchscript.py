...
import numpy as np
import torch
from src.network.covariance_parametrization import DiagonalParam
...
class MeasSourceTorchScript:
    ...
    def __init__(self, model_path, force_cpu=False):
        ...
        ts = time.time()
        warmup_input = torch.rand([1, 6, 200],device=self.device)
        print(f"Network inference time: {time.time() - ts}")
        y, y_cov = self.net(warmup_input)
