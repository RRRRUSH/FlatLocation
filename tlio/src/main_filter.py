...
if __name__ == "__main__":
    ...
    #         if len(s) > 0 and s[0] != "#"
    #     ]
    data_names = [item for item in glob.glob(os.path.join(args.root_dir, "*")) if os.path.isdir(item)]

    if len(data_names) == 0:
        logging.error("There are no datasets in the current directory!")
        exit(1)
