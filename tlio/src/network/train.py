...
# from open3d.cuda.pybind.pipelines.registration import L2Loss
...
# 定义一个函数来创建随机子集的数据加载器
def create_random_subset_loader(dataset, batch_size, subset_size):
    # 随机选择 subset_size 个索引
    indices = list(range(len(dataset)))
    random_subset_indices = random.sample(indices, subset_size)

    # 创建子集数据加载器
    sampler = SubsetRandomSampler(random_subset_indices)
    subset_loader = DataLoader(dataset, batch_size=batch_size, sampler=sampler)

    return subset_loader


def get_inference(network, data_set, device, epoch, transforms=[]):
    """
    ...
    l1s_all, l2s_all, l3s_all = [], [], []
    network.eval()

    # 随机选择一个子集大小
    subset_size = random.randint(1, len(data_set))  # 你可以根据需要调整范围

    # 创建随机子集的数据加载器
    val_loader = create_random_subset_loader(data_set, 512, subset_size)

    for bid, sample in enumerate(val_loader):
        ...
        l1, l2, l3, loss = get_loss(pred, pred_cov, targ, epoch)

        targets_all.append(torch_to_numpy(targ))
        preds_all.append(torch_to_numpy(pred))
        preds_cov_all.append(torch_to_numpy(pred_cov))
        l1s_all.append(torch_to_numpy(l1))
        l2s_all.append(torch_to_numpy(l2))
        l3s_all.append(torch_to_numpy(l3))
    ...
    l1s_all = np.concatenate(l1s_all, axis=0)
    l2s_all = np.concatenate(l2s_all, axis=0)
    l3s_all = np.concatenate(l3s_all, axis=0)
    attr_dict = {
        "targets": targets_all,
        "preds": preds_all,
...
def write_summary(summary_writer, attr_dict, epoch, optimizer, mode):
    ...
    l1_loss = np.average(attr_dict["l1s"])
    l2_loss = np.average(attr_dict["l2s"])
    l3_loss = np.average(attr_dict["l3s"])
    ...
    summary_writer.add_scalar(f"{mode}_dist/l1_loss", l1_loss, epoch)
    summary_writer.add_scalar(f"{mode}_dist/l2_loss", l2_loss, epoch)
    summary_writer.add_scalar(f"{mode}_dist/l3_loss", l3_loss, epoch)
...
def net_train(args):
    ...
    try:
        for epoch in range(start_epoch + 1, args.epochs):
            ...
            if val_loader is not None:
                val_attr_dict = get_inference(network, data.val_dataset, device, epoch)
        ...
        logging.info("-" * 30)
        logging.info("Early terminate")
        save_model(args, num_epoch, network, optimizer, best=False, interrupt=True)
        sys.exit()
    return
