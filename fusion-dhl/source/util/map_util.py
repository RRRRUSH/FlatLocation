...
from server.utils.coordinate_utils import transformer_wgs84_to_utm
...
def convert_to_utm(input_location):
    ...
    for location in input_location:
        utm_x, utm_y = transformer_wgs84_to_utm.transform(location[1], location[0])
        utm_data.append([utm_x, utm_y])
    # utm_data = [utm.from_latlon(location[0], location[1])[:2] for location in input_location]
    return np.asarray(utm_data)[:, :2].astype(np.float64)
...
def latlong_coordinates_on_map(map, dpi, map_coordinates, flp_data, visualize=True):
    ...
    # 使用 griddata 进行插值
    map_func_x = lambda x, y: griddata(utm_coords, map_coordinates[:, 0], (x, y), method='linear')
    map_func_y = lambda x, y: griddata(utm_coords, map_coordinates[:, 1], (x, y), method='linear')
