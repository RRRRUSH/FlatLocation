...
class FlowEvalDataset(Dataset):
    def __init__(self, data_path, floorplan_path, result_dpi=_dpi, image_dpi=_dpi, patch_size=250, buffer=10,
        ...
        if image_dpi != result_dpi:
            resized_size = (np.asarray(self.floorplan.shape[:2]) / image_dpi * result_dpi).astype(int)
        ...
        while idx[-1] < len(self.traj) - 1:
            ...
            i = max(idx[-1] - int(traj_length // 4), i + int(traj_length // 4),i+1)
        
        

        self.data_list = np.asarray(self.data_list).astype(int)
    ...
    def get_traj_segment(self, index):
        ...
        return np.round(traj_segment).astype(int)
