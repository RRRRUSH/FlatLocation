...
class ImuTracker:
    ...
    def on_imu_measurement(self, t_us, gyr_raw, acc_raw):
        assert isinstance(t_us, int)
        self.imu_index = t_us
        if t_us - self.last_t_us > 5e3: ## disabling for 100Hz seq
            # 如果丢包，返回丢包时长
            self.imu_gap = t_us - self.last_t_us

        if self.filter.initialized:
            return self._on_imu_measurement_after_init(t_us, gyr_raw, acc_raw), self.imu_index
        else:
            self._init_without_state_at_time(t_us, gyr_raw, acc_raw)
            return False, self.imu_index
