...
class ImuTrackerRunner:
    ...
    def add_data_to_be_logged(self, ts, acc, gyr, with_update, imu_index):
        # filter data logger
        ...
        temp = np.concatenate(
            [
                ...
                inno_sigma,
                sigmasyawp,
                np.array(imu_index).reshape(-1, 1) # 检查imu是否丢包
            ],
            axis=0,
        )
    ...
    def run_tracker(self, args):
        ...
        for i in tqdm(range(n_data)):
            ...
            if self.tracker.filter.initialized:
                did_update, imu_index = self.tracker.on_imu_measurement(t_us, gyr_raw, acc_raw)
