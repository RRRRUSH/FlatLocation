...
from llio.model_twolayer import TwoLayerModel
...
class NonLinearity(nn.Module):
    ...
    def forward(self, vector, scalar):
        ...
        return x[..., :self.dim_out].unsqueeze(-2) * (
                    vector / torch.norm(vector, dim=-2).clamp(min=1e-6).unsqueeze(-2)), x[..., -self.scalar_dim_out:]
...
class Convolutional(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.conv_layer_vec = nn.Conv2d(in_channels=dim_in, out_channels=dim_out, stride=stride, kernel_size=kernel,
                                        padding=padding, bias=bias, padding_mode='replicate')
        self.conv_layer_sca = nn.Conv2d(in_channels=scalar_dim_in, out_channels=scalar_dim_out, stride=stride,
                                        kernel_size=kernel, padding=padding, bias=bias, padding_mode='replicate')

    def forward(self, vector, scalar):
        return self.conv_layer_vec(vector.permute(0, 3, 1, 2)).permute(0, 2, 3, 1), self.conv_layer_sca(
            scalar.unsqueeze(-2).permute(0, 3, 1, 2)).permute(0, 2, 3, 1).squeeze(-2)


class Eq_Motion_Model_fullCov(nn.Module):  ## input vector and scalar separately 默认框架 eq_o2_frame_fullCov_2vec_2deep
    def __init__(
            self,
            dim_in,
            dim_out,
        ...
        self.nonlinearity0 = NonLinearity(dim_in=hidden_dim, dim_out=hidden_dim, scalar_dim_in=scalar_hidden_dim,
                                          scalar_dim_out=scalar_hidden_dim)

        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Convolutional(dim_in=hidden_dim, dim_out=hidden_dim, scalar_dim_in=scalar_hidden_dim,
                              scalar_dim_out=scalar_hidden_dim, stride=stride, padding=padding, kernel=kernel,
                              bias=bias),
                NonLinearity(dim_in=hidden_dim, dim_out=hidden_dim, scalar_dim_in=scalar_hidden_dim,
                             scalar_dim_out=scalar_hidden_dim),
        ...
        self.nonlinearity1 = NonLinearity(dim_in=hidden_dim, dim_out=hidden_dim, scalar_dim_in=scalar_hidden_dim,
                                          scalar_dim_out=0)
        ...
        # ## TLIO
        # self.tlio = ResNet1D(BasicBlock1D, tlio_in_dim, tlio_out_dim, tlio_depths, tlio_net_config_in_dim,
        #                       cov_output_dim=tlio_cov_dim_out)
        ...
        self.llio = TwoLayerModel(model_para)
...
if __name__ == "__main__":  ## to quickly check
    ...
    eq_model = Eq_Motion_Model_fullCov(dim_in=3, dim_out=2, scalar_dim_in=9, pooling_dim=1,
                                       tlio_in_dim=6, tlio_out_dim=3, tlio_depths=[2, 2, 2, 2],
                                       tlio_net_config_in_dim=200 // 32 + 1, tlio_cov_dim_out=3,
                                       hidden_dim=128, scalar_hidden_dim=128, depth=2, stride=1, padding='same',
                                       kernel=(32, 1), bias=False
                                       )
    eq_model.eval()
