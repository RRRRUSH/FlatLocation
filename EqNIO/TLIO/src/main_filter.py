...
if __name__ == "__main__":
    ...
    add_bool_arg(io_groups, "erase_old_log", default=True)
    ...
    # TODO 使用自定义的 allan 方差初始化滤波器
    # --sigma_na 1.1212224622294688e-02 --sigma_ng 1.1752829958889025e-03 --ita_ba 3.4661445096824021e-04 --ita_bg 5.6034859604820343e-06
    filter_group.add_argument(
        "--sigma_na", type=float, default=np.sqrt(1e-3) # np.sqrt(1e-3)
    )  # accel noise  m/s^2
    filter_group.add_argument(
        "--sigma_ng", type=float, default=np.sqrt(1e-4) # np.sqrt(1e-4)
    )  # gyro noise  rad/s
    filter_group.add_argument(
        "--ita_ba", type=float, default=1e-4 # 1e-4
    )  # accel bias noise  m/s^2/sqrt(s)
    filter_group.add_argument(
        "--ita_bg", type=float, default=1e-6 # 1e-6
    ...
    data_list = glob.glob(os.path.join(args.root_dir, "*list.txt"))[0]
