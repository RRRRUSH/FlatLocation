...
class FCOutputModule(nn.Module):
    ...
    def __init__(self, in_planes, num_outputs, model_params):
        """
        ...
        trans_planes = model_params['trans_planes']
...
_blocks = {
    'BasicBlock1D': BasicBlock1D,
    'FCOutputModule': FCOutputModule,
}


class RONINResNet1D(nn.Module):
    def __init__(self, model_params):
        super(RONINResNet1D, self).__init__()
        num_inputs = model_params['num_inputs']
        num_outputs = model_params['num_outputs']
        group_sizes = model_params['group_sizes']
        self.base_plane = model_params['base_plane']
        block_type = _blocks[model_params['block_type']]
        output_block = _blocks[model_params['output_block']]
        zero_init_residual = model_params['zero_init_residual']
        ...
        if output_block is None:
            ...
            self.output_block = output_block(self.planes[-1] * block_type.expansion, num_outputs, model_params)
